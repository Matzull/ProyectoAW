...
$start = $seg->getstart_time();
$unix_epoc_start = strtotime($start) * 1000 + explode(".", $start)[1];

$end = \parallelize_namespace\Aplicacion::getTime();
$unix_epoc_end = strtotime($end) * 1000 + explode(".", $end)[1];
$tiempo_ejecucion = $unix_epoc_end - $unix_epoc_start;

$usuario->setMillisCrunched($usuario->getMsCrunched() + $tiempo_ejecucion);